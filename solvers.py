...
def newtons_error_series(f: Callable[[float], float], f_prime: Callable[[float], float], x: float, max_iterations: int, tolerance: float) -> float:
    errors = [x]
    for _ in range(max_iterations):
        x = x - f(x)/f_prime(x)
        errors.append(x)
...
def secant_error_series(f: Callable[[float], float], x_0: float, x_1: float, max_iterations: int, tolerance: float) -> float:
    errors = [x_0, x_1]
    for _ in range(max_iterations):
        swap = x_1
        x_1 = x_1 - (f(x_1)*(x_1-x_0))/(f(x_1)-f(x_0))
...
def bisection_error_series(f: Callable[[float], float], a: float, b: float, max_iterations: int, tolerance: float) -> float:
    def sign(y: float) -> int:
        if y >= 0:
            return 1
        return -1
